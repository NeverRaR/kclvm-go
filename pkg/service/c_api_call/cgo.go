...
func loadKclvmServiceCapiLib() *dlopen.LibHandle {
	...
	if !ok {
		panic(fmt.Errorf("kclvm_capi : can't find src path of **/service/c_api_call/cgo.go "))
	}
	curDir := strings.TrimSuffix(srcFilePath, "/cgo.go")
	...
	libPath := strings.Join([]string{curDir, "packaged/lib", osWithArch, "libkclvm_capi" + libSuffix}, "/")
